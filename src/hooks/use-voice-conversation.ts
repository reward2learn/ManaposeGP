...
const RECOGNITION_RESTART_MS = 100;
const RAPID_RESTART_WINDOW_MS = 400;
...
export function useVoiceConversation({
  ...
  const enableVoiceMode = useCallback(async () => {
    ...
    // Skip getUserMedia — SpeechRecognition handles audio capture natively.
    // getUserMedia triggers a slow browser permission dialog that blocks mic activation.
    micUnavailableRef.current = false;
    rapidRestartCountRef.current = 0;
