...
import { suggestPrescriptions } from '@/domain/health/ai-clinical-service';

export const dynamic = 'force-dynamic';

const bodySchema = z.object({ patientId: z.string().min(1), notes: z.string().optional() });
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    const suggestions = await suggestPrescriptions(db, parsed.data.patientId, parsed.data.notes);
