...
const chatBodySchema = z.object({
  ...
  mode: z.enum(['health-patient']).optional(),
});
...
const HEALTH_PATIENT_SYSTEM_PROMPT = [
  "You are ManaposeGP Health Assistant — an educational resource about women's health and menopause. You provide evidence-based information from Jean Hailes for Women's Health, the Australasian Menopause Society, and the RACGP.",
  'You NEVER diagnose, prescribe medication, or replace medical advice. Always encourage users to discuss concerns with their GP.',
  'If a user describes urgent symptoms (postmenopausal bleeding, chest pain, severe depression with suicidal thoughts), advise them to seek immediate medical attention. Provide emergency contacts: Lifeline 13 11 14, 000 for emergencies.',
  'Explain concepts in plain language. Cite sources when possible. Be empathetic and supportive.',
  'When talking about treatments, explain both hormonal (MHT) and non-hormonal options, and always note that treatment decisions should be made with a GP.',
].join('\n\n');

async function fetchHealthEducationContext(db: ReturnType<typeof createClient>): Promise<string> {
  try {
    const snippets = await db.knowledgeSnippet.findMany({
      where: { category: { startsWith: 'health_' } },
      take: 3,
      orderBy: { key: 'asc' },
    });
    if (snippets.length) {
    ...
    // non-fatal — health context is supplementary
  }
  return '';
}
...
/**
 * Fetch relevant knowledge from the vector store for a user query.
 * Returns a formatted context string or empty string.
 */
async function fetchRagContext(query: string, isHealthMode: boolean): Promise<string> {
  try {
    const snippets = await searchKnowledgeBase(query, isHealthMode ? 3 : 2);
    return formatKnowledgeContext(snippets);
  } catch {
    return ''; // Non-fatal — RAG is an enhancement, not a requirement
  }
}
...
async function handleChatPost(request: Request): Promise<Response> {
  ...
  try {
    ...
    const isHealthMode = parsed.data.mode === 'health-patient';

    let systemPrompt: string;
    if (isHealthMode) {
      const healthContext = await fetchHealthEducationContext(db);
      systemPrompt = healthContext
        ? `${HEALTH_PATIENT_SYSTEM_PROMPT}\n\n=== HEALTH EDUCATION CONTEXT ===\n${healthContext}`
        : HEALTH_PATIENT_SYSTEM_PROMPT;
    } else {
      systemPrompt = await knowledge.buildSystemPrompt();
    }
    ...
    const systemSections = [
      systemPrompt,
      // Inject relevant blog/conversation knowledge via vector search
      await fetchRagContext(message, isHealthMode),
