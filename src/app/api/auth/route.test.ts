...
describe('/api/auth', () => {
  ...
  it('UC-AUTH-02: verify-pin rejects incorrect PIN', async () => {
    ...
    expect(json.error).toBe('Incorrect code');
  });

  it('UC-AUTH-02: verify-pin accepts default code 454212 when no ADMIN_PIN stored', async () => {
    vi.mocked(getSecretPlaintext).mockResolvedValue(null); // No stored secret
    const response = await POST(
      new Request('http://localhost/api/auth?action=verify-pin', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ pin: '454212' }),
      }),
    );
    expect(response.status).toBe(200);
    const setCookie = response.headers.get('set-cookie') ?? '';
  ...
  it('UC-AUTH-02: verify-pin rejects wrong code even with default fallback', async () => {
    vi.mocked(getSecretPlaintext).mockResolvedValue(null); // No stored secret
    const response = await POST(
      new Request('http://localhost/api/auth?action=verify-pin', {
        method: 'POST',
  ...
  it('UC-AUTH-04b: logout-client returns JSON and clears session cookie', async () => {
    const response = await GET(new Request('http://localhost/api/auth?action=logout-client'));
    expect(response.status).toBe(200);
    const json = await response.json() as { ok: boolean };
    expect(json.ok).toBe(true);
    const setCookie = response.headers.get('set-cookie') ?? '';
