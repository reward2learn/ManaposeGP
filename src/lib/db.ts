...
function getBasePrisma(): PrismaClient {
  if (!globalForPrisma.__manaposePrisma) {
    globalForPrisma.__manaposePrisma = new PrismaClient({
      datasources: { db: { url: getPostgresUrl() } },
    });
  }
  return globalForPrisma.__manaposePrisma;
}
