...
export const blockConfigSchemas = {
  ...
  // Healthcare block schemas (Phase 0 — minimal config)
  health_metrics_cards: z.object({ variant: z.string().optional(), minTier: minTierSchema }).passthrough(),
  symptom_timeline: z.object({ period: z.string().optional(), minTier: minTierSchema }).passthrough(),
  ai_insights_panel: z.object({ mode: z.string().optional(), minTier: minTierSchema }).passthrough(),
  daily_symptom_form: z.object({ minTier: minTierSchema }).passthrough(),
  symptom_trends_chart: z.object({ period: z.string().optional(), minTier: minTierSchema }).passthrough(),
  gp_summary_generator: z.object({ format: z.string().optional(), minTier: minTierSchema }).passthrough(),
  consultation_checklist: z.object({ minTier: minTierSchema }).passthrough(),
  symptom_summary_export: z.object({ format: z.string().optional(), minTier: minTierSchema }).passthrough(),
  patient_list: z.object({ minTier: minTierSchema }).passthrough(),
  clinical_alerts: z.object({ minTier: minTierSchema }).passthrough(),
  guideline_search: z.object({ sources: z.array(z.string()).optional(), minTier: minTierSchema }).passthrough(),
  guideline_browser: z.object({ sources: z.array(z.string()).optional(), minTier: minTierSchema }).passthrough(),
  drug_interaction_checker: z.object({ minTier: minTierSchema }).passthrough(),
  risk_calculator: z.object({ models: z.array(z.string()).optional(), minTier: minTierSchema }).passthrough(),
  patient_context_panel: z.object({ minTier: minTierSchema }).passthrough(),
  soap_note_generator: z.object({ minTier: minTierSchema }).passthrough(),
  referral_letter_generator: z.object({ directory: z.string().optional(), minTier: minTierSchema }).passthrough(),
  health_education_library: z.object({ topics: z.array(z.string()).optional(), minTier: minTierSchema }).passthrough(),
